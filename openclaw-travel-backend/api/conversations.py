...
from database import ConversationRecord, get_session
...
async def list_conversations(
    ...
    records = db.exec(
        select(ConversationRecord).order_by(ConversationRecord.updated_at.desc())  # type: ignore[arg-type]
    ).all()
    return [
        ConversationSummary(
...
async def update_conversation(
    ...
    record = db.exec(
        select(ConversationRecord).where(ConversationRecord.conversation_id == conv_id)
    ).first()
    if record is None:

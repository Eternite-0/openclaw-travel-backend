...
def _clean_candidate_text(text: str) -> list[str]:
    ...
    # Drop common action prefixes generated by LLM activity sentences.
    t = re.sub(
        r"^(早餐后从市区出发前往|前往并游览|前往并参观|前往|游览|游玩|乘车前往|返回|在|收官晚餐|午餐后前往)",
        "",
        t,
    ).strip()
    ...
    for p in parts:
        ...
        if not p:
            continue
        # Keep concise geocodable chunks.
        if len(p) > 40:
            p = p[:40]
        if len(p) < 2:
            continue
...
def _build_geocode_candidates(name: str, loc: str, city: str) -> list[str]:
    candidates: list[str] = []
    raw_items = [loc, name]
    ...
    for raw in raw_items:
        if not raw:
            continue
        for token in _clean_candidate_text(raw):
            candidates.append(token)
            if city:
                candidates.append(f"{city}{token}")

    # Extra fallbacks for "机场周边/市区酒店" style vague locations.
    loc_l = (loc or "").lower()
    if city and ("机场" in loc_l):
        candidates.append(f"{city}机场")
    if city and ("市区" in loc_l):
        candidates.append(f"{city}市中心")
...
async def geocode_itinerary_activities(itinerary, city: str = "") -> tuple[int, int]:
    ...
    for day in itinerary.days:
        for act in day.activities:
            ...
            candidates = _build_geocode_candidates(name=name, loc=loc, city=city)
            ...
            has_old = bool(old_lat and old_lng)
            ...
            if best_result:
                # Too-far corrections are likely wrong matches; keep original.
                if has_old and best_dist > _MAX_ACCEPT_SHIFT_M:
                    logger.warning(
                        "Geocode rejected '%s': best shift %.0fm too large, keep original",
                        name[:30], best_dist,
                    )
                    fail += 1
                    continue

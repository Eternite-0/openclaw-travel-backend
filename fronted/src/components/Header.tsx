...
export const Header = memo(function Header({ onMenuClick }: HeaderProps) {
  return (
    <header className="fixed top-0 w-full z-50 bg-surface-container-lowest/80 backdrop-blur-md flex justify-between items-center px-4 md:px-6 h-16 border-b border-outline-variant/20">
      <div className="flex items-center gap-4 md:gap-8">
        <button 
          onClick={onMenuClick}
          className="lg:hidden p-2 -ml-2 rounded-lg hover:bg-surface-container-high transition-colors"
          aria-label="打开菜单"
        >
          <Menu className="w-5 h-5 text-on-surface" />
        </button>
        <div className="flex items-center gap-2">
          <span className="text-base md:text-lg font-bold text-on-surface tracking-tight">OpenTrip</span>
          <span className="hidden sm:inline text-[10px] font-bold px-2 py-0.5 bg-surface-container-high text-on-surface-variant rounded-full tracking-widest">TRAVEL PLANNER</span>
        </div>
